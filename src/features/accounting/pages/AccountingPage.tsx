...
export default function AccountingPage(): React.ReactElement {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ['accounting-periods', page],
    queryFn: () => getAccountingPeriods({ page, per_page: 15 }),
  });
  ...
  return (
    <div className="p-6 space-y-5">
      <header>
        ...
        {result && (
          <p className="text-sm text-gray-500 mt-0.5">{result.pagination.total} periods</p>
        )}
      </header>
      ...
      {isError && (
        <div role="alert" className="rounded-lg bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">
          Failed to load accounting periods. Please try again.
        </div>
      )}
            ...
            <table className="min-w-full divide-y divide-gray-100 text-sm">
              <thead>
                <tr className="bg-gray-50 text-xs font-semibold text-gray-500 uppercase tracking-wide">
                  ...
                  <th className="px-5 py-3 text-left">Closed At</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {result.periods.length === 0 ? (
                  ...
                  result.periods.map((p: AccountingPeriod) => (
                    <tr key={p.id} className="hover:bg-gray-50">
                      <td className="px-5 py-3 font-medium text-gray-900">{p.name}</td>
                      <td className="px-5 py-3 text-gray-500 text-xs">{new Date(p.start_date).toLocaleDateString()}</td>
                      <td className="px-5 py-3 text-gray-500 text-xs">{new Date(p.end_date).toLocaleDateString()}</td>
                      ...
                      <td className="px-5 py-3 text-gray-500 text-xs">
                        {p.closed_at ? new Date(p.closed_at).toLocaleDateString() : <span className="text-gray-300">—</span>}
                      </td>
                    </tr>
                  ))
                )}
