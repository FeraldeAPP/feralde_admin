...
import type { Wallet, WalletFilters } from '../types';
...
export default function WalletsPage() {
  ...
  const { data: walletsData, isLoading: loadingWallets } = useWallets(filters);
  const { data: statsData } = useWalletStats();

  const result = walletsData?.success ? walletsData.data : null;
  ...
  const columns = [
    ...
    columnHelper.accessor('id', {
      ...
      cell: (info) => <span className="font-medium text-stone-900">#W-{info.getValue()}</span>,
    }),
        ...
        return (
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full overflow-hidden bg-stone-100">
              <img src={owner?.avatar || '/user.png'} alt={owner?.name} className="w-full h-full object-cover" />
            </div>
            <span className="text-stone-900 font-medium">{owner?.name || 'Unknown User'}</span>
          </div>
        );
      },
    }),
    columnHelper.accessor('owner.type', {
      id: 'account_type',
      header: 'Account Type',
      cell: (info) => (
        <span className={cn(
          "px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider",
          info.getValue() === 'Distributor' ? "bg-indigo-50 text-indigo-600" : "bg-purple-50 text-purple-600"
        )}>
          {info.getValue()}
        </span>
      ),
    ...
    columnHelper.accessor('balance', {
      header: 'Balance (P)',
      cell: (info) => <span className="text-stone-900 font-bold">{parseFloat(info.getValue()).toLocaleString()}</span>,
    }),
    columnHelper.accessor('pending_balance', {
      header: 'Pending (P)',
      cell: (info) => <span className="text-amber-600 font-medium">{parseFloat(info.getValue()).toLocaleString()}</span>,
    }),
    columnHelper.accessor('last_transaction', {
      header: 'Last Transaction',
      cell: (info) => <span className="text-stone-500">{info.getValue() || 'N/A'}</span>,
    }),
      ...
      cell: (info) => {
        ...
        return (
          <span className={cn(
            ...
            status === 'SUSPENDED' && "bg-red-50 text-red-600"
          )}>
            {status}
          </span>
        );
  ...
  return (
    <div className="p-4 sm:p-6 space-y-6 bg-white min-h-screen font-sans">
      ...
      <h1 className="text-xl sm:text-2xl font-bold text-stone-900 -mt-2">Wallets Management</h1>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 pt-2">
        <div className="relative flex-1 sm:max-w-sm">
          ...
          <Input
            placeholder="Search wallets, users..."
            value={search}
        ...
        <div className="flex items-center gap-2 sm:ml-auto">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              <DropdownMenuItem>By Account Type</DropdownMenuItem>
              <DropdownMenuItem>By Status</DropdownMenuItem>
              <DropdownMenuItem>Low Balance</DropdownMenuItem>
            </DropdownMenuContent>
          </DropdownMenu>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          value={stats?.total_platform_balance.amount.toLocaleString() ?? '0'}
          trend={stats?.total_platform_balance.trend ?? 0}
          chart="/expenses/chart-up.svg"
          type="bar"
        />
        <StatCard
          title="Total Pending"
          value={stats?.total_pending.amount.toLocaleString() ?? '0'}
          trend={stats?.total_pending.trend ?? 0}
          chart="/expenses/chart-pie.svg"
          type="pie"
          percentage={stats?.total_pending.percentage}
        />
        <StatCard
          title="Average Wallet Balance"
          value={stats?.average_balance.amount.toLocaleString() ?? '0'}
          trend={stats?.average_balance.trend ?? 0}
          chart="/expenses/chart-line-1.svg"
          type="area"
        />
        <StatCard
          title="Active Wallets"
          value={stats?.active_wallets.count.toLocaleString() ?? '0'}
          trend={stats?.active_wallets.trend ?? 0}
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            <div className="flex items-baseline gap-1">
              <span className="text-xl font-bold text-stone-900">{title.includes('Wallets') ? '' : '₱'} {value}</span>
            </div>
            <div className="flex items-center gap-2">
