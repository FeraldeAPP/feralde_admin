...
function BundleModal({ mode, onClose }: {
    ...
    const createMutation = useMutation({
        ...
        onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['bundles'] }); onClose(); },
    });

    const updateMutation = useMutation({
        mutationFn: ({ id, payload }: { id: number; payload: FormValues }) => updateBundle(id, payload),
        onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['bundles'] }); onClose(); },
    });
...
export default function BundlesPage(): React.ReactElement {
    ...
    const deleteMutation = useMutation({
        ...
        onSuccess: () => { void queryClient.invalidateQueries({ queryKey: ['bundles'] }); },
    });
                        ...
                        <table className="min-w-full text-sm">
                            <thead>
                                <tr className="border-b border-stone-100">
                                    ...
                                    <th className="pb-3 text-left text-xs font-medium text-stone-400 pr-6 hidden sm:table-cell">Included Products</th>
                                    <th className="pb-3 text-left text-xs font-medium text-stone-400 pr-6">Total Price (₱)</th>
                            ...
                            <tbody>
                                {filtered.length === 0 ? (
                                    ...
                                    filtered.map((bundle: Bundle) => (
                                        <tr key={bundle.id} className="border-b border-stone-100 hover:bg-stone-50 transition-colors">
                                            {/* Bundle Name + icon */}
                                            ...
                                            {/* Included Products — placeholder since bundle items aren't in current type */}
                                            ...
                                            <td className="py-3.5 pr-6">
                                                <span className="text-sm text-stone-800 font-medium">
                                                    {parseFloat(bundle.retail_price).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
                                                </span>
                                            </td>
                                            {/* Action */}
